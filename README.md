...
3. Create file(s) containing lists of node names - one fully qualified node name, as shown by `kubectl get nodes`, per line. This script lists the node names of the specified nodepool:
```
...
The script launches a background process for each node listed in NODEFILE. Each process:
  ...
  - Drains remaining non-daemonset pods from the nodes
  - Deletes the node, then finishes.
Subsequently, and outside of the script's scope, Crusoe Managed Kubernetes automatically creates a new node, with a new name, to replace the deleted node.  
